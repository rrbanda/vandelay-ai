#!/usr/bin/env python3
"""
Migration Vector Store Ingestion Script
=========================================

Standalone script to ingest VMware to BareMetal OpenShift migration documents
into LlamaStack Vector Store.

This script is separate from the FSI vector ingestion and handles migration-
specific documents including:
- Migration overview and prerequisites
- CD tool-specific guides
- Certificate, secrets, and SSO configuration
- Network, storage, and vanity URL guides
- Validation checklists and FAQ

Features:
- Creates separate vector store for migration content
- Clears existing migration data before ingestion (optional)
- Supports data from git repository URL or local files
- Can run on container startup

Usage:
    # Use local data (default)
    python -m data_ingestion.ingest_migration_vector
    
    # Clear existing migration vector data first
    python -m data_ingestion.ingest_migration_vector --clear
    
    # Use data from git URL
    python -m data_ingestion.ingest_migration_vector --git-url https://github.com/user/repo.git
    
    # Specify documents directory
    python -m data_ingestion.ingest_migration_vector --docs-dir data/migration_documents

Environment Variables:
    LLAMASTACK_BASE_URL: LlamaStack server URL
    MIGRATION_VECTOR_STORE_ID: Migration vector store ID (created if not exists)
...
# Default vector store ID for migration documents
DEFAULT_MIGRATION_VECTOR_STORE_ID = "migration_docs_store"
DEFAULT_MIGRATION_VECTOR_STORE_NAME = "Migration Documentation Store"
...
def get_migration_vector_store_config() -> dict:
    """
    Get vector store configuration for migration documents.
    
    Uses separate environment variables for migration vector store,
    falling back to defaults if not set.
    
    Returns:
        Dict with base_url, vector_store_id, vector_store_name, embedding_model
    """
    # Get base config
    base_config = get_vector_store_config()
    
    # Override with migration-specific settings
    return {
        'base_url': base_config.get('base_url', ''),
        'vector_store_id': os.environ.get(
            'MIGRATION_VECTOR_STORE_ID',
            DEFAULT_MIGRATION_VECTOR_STORE_ID
        ),
        'vector_store_name': os.environ.get(
            'MIGRATION_VECTOR_STORE_NAME',
            DEFAULT_MIGRATION_VECTOR_STORE_NAME
        ),
        'embedding_model': base_config.get('embedding_model', ''),
    }


def run_migration_vector_ingestion(
    git_url: str = None,
    git_branch: str = "main",
    docs_dir: str = "data/migration_documents",
    ...
    """
    Run the migration vector store ingestion pipeline.
    
    Args:
        git_url: Git repository URL for data source (None for local)
        git_branch: Git branch to use
        docs_dir: Directory containing migration documents
    ...
    if verbose:
        print("=" * 60)
        print("MIGRATION VECTOR STORE INGESTION")
        print("VMware OpenShift → BareMetal OpenShift Guides")
        print("=" * 60)
    
    # Get migration-specific config
    config = get_migration_vector_store_config()
    base_url = config.get('base_url', '')
    vector_store_id = config.get('vector_store_id', '')
    vector_store_name = config.get('vector_store_name', '')
    ...
    # Create loader with migration-specific vector store
    ...
    try:
        ...
        if not loader.ensure_vector_store():
            return {'success': False, 'error': 'Failed to ensure migration vector store exists'}
        
        # Clear if requested
        if clear_first:
            if verbose:
                print("Clearing migration vector store...")
        ...
        # Load documents
        if verbose:
            print(f"\nLoading migration documents from: {docs_dir}")
        ...
        if verbose:
            print(f"Found {len(documents)} migration documents:")
            for doc in documents:
                doc_name = doc.get('name', doc.get('filename', 'unknown'))
                print(f"  - {doc_name}")
        
        # Ingest documents with migration-specific type
        result = loader.load_documents(
            documents,
            doc_type="migration_document",  # Migration-specific type
        ...
        if verbose:
            print("\n" + "=" * 60)
            print("MIGRATION VECTOR INGESTION COMPLETE!")
            print("=" * 60)
            if final_store_id:
                print(f"\n[INFO] Migration vector store: {final_store_id}")
                print(f"[INFO] Set MIGRATION_VECTOR_STORE_ID={final_store_id} to reuse")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Migration Vector Ingestion - Load migration docs into LlamaStack"
    )
    ...
    parser.add_argument(
        "--docs-dir",
        type=str,
        default="data/migration_documents",
        help="Directory containing migration documents"
    )
    parser.add_argument(
        "--docs-pattern",
        type=str,
    ...
    result = run_migration_vector_ingestion(
